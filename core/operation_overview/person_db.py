...
ALLOWED_COLUMNS = {"name", "primary_name", "birthdate", "gender", "hurt", "handover", "info", "triage"}
...
def add_person(mission_id, name, primary_name, birthdate, gender, handover, info) -> dict:
    con = get_db()
    number = con.execute("SELECT COALESCE(MAX(number), 0) + 1 AS n FROM overview_persons").fetchone()["n"]
    cur = con.execute("INSERT INTO overview_persons (mission_id, number, name, primary_name, birthdate, gender, handover, info) VALUES (?,?,?,?,?,?,?,?)", (mission_id, number, name, primary_name, birthdate, gender, handover, info))
    con.commit()
